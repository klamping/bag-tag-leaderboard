...
const { nextId, nextNumber } = require("./data/idGenerator");
...
async function insertPlayer(payload) {
  const player = {
    ...payload,
    id: nextId(EVENTS_DATA.players, "player"),
  };
...
async function insertConfirmedEvent(payload) {
  const event = {
    ...payload,
    id: nextId(EVENTS_DATA.events, "evt_confirmed"),
    confirmed: true,
    status: "confirmed",
  };
...
async function insertEventResults(rows) {
  const startingNumber = nextNumber(EVENTS_DATA.eventResults, "result");
...
async function insertEventPoints(rows) {
  const startingNumber = nextNumber(EVENTS_DATA.eventPoints, "point");
  const insertedRows = rows.map((row, index) => ({
    ...row,
