...
function validateParticipants(participants) {
  ...
  for (let index = 0; index < participants.length; index += 1) {
    ...
    if (!Number.isInteger(participant.finishPlace) || participant.finishPlace < 1) {
      throw new Error(
        `Participant ${participant.playerId} has invalid finishPlace; expected integer >= 1`
      );
    }

    if (participant.startingTag !== undefined) {
...
/**
 * @param {{isMajor?: boolean, participants: Array<{playerId: string, finishPlace: number, startingTag?: number}>}} event
 * @returns {Array<{
 *   playerId: string,
 *   attendance: number,
 *   placement: number,
...
function scoreEvent(event) {
  ...
  return participants.map((participant, index) => {
    const attendance = 2;
    const placement = getPlacementPoints(participant.finishPlace, fieldSize);
    ...
    const beatYourTagBonus = hasStartingTag && Number.isInteger(startingRank)
