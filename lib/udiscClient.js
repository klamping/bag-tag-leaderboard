...
function parseStructuredPayload(html) {
  ...
  function parseJsonCandidate(script) {
    ...
    const assignmentMatch = script.match(/=\s*([\s\S]*?);\s*$/);
    if (assignmentMatch) {
...
function parseUdiscEventPayloadOrThrow(html) {
  ...
  const fallback = structured || parseFallbackPayload(html);

  const participants = dedupeParticipantsOrThrow(fallback.participants || []);
  const payload = {
    name: stripTags(fallback.name),
    date: normalizeDate(fallback.date),
    participants,
  };

  if (fallback.slug) {
    payload.slug = fallback.slug;
  }
  if (fallback.isMajor === true) {
