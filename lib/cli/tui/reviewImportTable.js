...
function formatRow(row, { bootstrap = false } = {}) {
  const columns = [
    row.playerName,
    row.matchStatus,
    row.importedResult,
...
async function reviewImportTable({ event, rows, bootstrap = false, session, input = process.stdin, output = process.stdout }) {
  let rl = null;
  ...
  try {
    for (const row of rows) {
      output.write(`${formatRow(row, { bootstrap })}\n`);
